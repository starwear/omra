    ...
    try:
        while True:
            ...
            if unbuilded_header.get("command") == MRIM_CS_HELLO:
                ...
                # Передача файлов
                logger.info("Получил команду MRIM_CS_FILE_TRANSFER от клиента {}".format(address[0]))
                ...
                # Передача файлов
                logger.info("Получил команду MRIM_CS_FILE_TRANSFER_ACK от клиента {}".format(address[0]))
...
async def file_transfer(data, proto, email):
    ...
    for client in clients.values():
        if client.get("email") == parsed_data.get("email"):
            ...
            client.get("writer").write(response)
            await client.get("writer").drain()
            logger.info("Отправил команду MRIM_CS_FILE_TRANSFER клиенту {}".format(client.get("email")))
...
async def file_transfer_ack(data, proto, email):
    ...
    for client in clients.values():
        if client.get("email") == parsed_data.get("email"):
            ...
            client.get("writer").write(response)
            await client.get("writer").drain()
            logger.info("Отправил команду MRIM_CS_FILE_TRANSFER_ACK клиенту {}".format(client.get("email")))
